local Migrations = {
  {
    name = "m2015-09-16-132400_init_hmacauth",
    up = function(options)
      return [[
         CREATE TABLE IF NOT EXISTS hmacauth_credentials(
          id uuid,
          consumer_id uuid REFERENCES consumers (id) ON DELETE CASCADE,
          username text,
          secret text,
          created_at timestamp without time zone default (now() at time zone 'utc'),
          PRIMARY KEY (id)
        );

        DO $$
        BEGIN
        IF (
          SELECT to_regclass('public.hmacauth_credentials_username')
            ) IS NULL THEN
          CREATE INDEX hmacauth_credentials_username ON hmacauth_credentials(username);
        END IF;

        IF (
          SELECT to_regclass('public.hmacauth_credentials_consumer_id')
            ) IS NULL THEN
          CREATE INDEX hmacauth_credentials_consumer_id ON hmacauth_credentials(consumer_id);
        END IF;
        END$$;
      ]]
    end,
    down = function(options)
      return [[
        DROP TABLE hmacauth_credentials;
      ]]
    end
  }
}

return Migrations
