  ...
  describe("Base DAO", function()
    ...
    describe(":update()", function()
      ...
      it("should update specified, non-primary fields in DB", function()
        ...
        apis, rows = session:query("SELECT * FROM apis WHERE name = '"..api_t.name.."'")
        assert.falsy(not apis)
        assert.equal(1, #apis)
        ...
        -- Consumer
        local consumers, rows = session:query("SELECT * FROM consumers")
        ...
        consumers, rows = session:query("SELECT * FROM consumers WHERE custom_id = '" ..consumer_t.custom_id.."'")
        assert.falsy(not consumers)
        ...
        plugins, rows = session:query("SELECT * FROM plugins WHERE id = '" .. plugin_t.id .. "'")
        assert.falsy(not plugins)
      ...
      describe("full", function()
        ...
        it("should set to NULL if a field is not specified", function()
          ...
          -- Check update
          api, rows = session:query("SELECT * FROM apis WHERE id = '"..api.id.."'")
        ...
        it("should still check the validity of the schema", function()
          ...
          -- Check update failed
          api, row = session:query("SELECT * FROM apis WHERE id = '"..api.id.."'")
          assert.falsy(not api)
    ...
    describe(":find()", function()
      ...
      describe_core_collections(function(type, collection)
        ...
        it("should find entities", function()
          local entities, row = session:query("SELECT * FROM "..collection)
    ...
    describe(":delete()", function()
      ...
      describe_core_collections(function(type, collection)
        ...
        it("should delete an entity based on its primary key", function()
          local entities, rows = session:query("SELECT * FROM "..collection)
          assert.falsy(not entities)
          assert.truthy(entities)
          assert.True(#entities > 0)
          ...
          local entities, rows = session:query("SELECT * FROM "..collection.." WHERE id = '".. entities[1].id.."'")
          assert.falsy(not entities)
          assert.truthy(entities)
